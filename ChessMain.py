...
import pygame.transform as ptrans
...
def loadImages():
    ...
    # Tạo một từ điển để lưu trữ hình ảnh của các quân cờ
    IMAGES = {}
    
    # Duyệt qua danh sách các quân cờ và tải hình ảnh của chúng
    for piece in pieces:
        # Tải hình ảnh và điều chỉnh kích thước
        image = ptrans.scale(pimg.load(IMAGE_PATH + piece + ".png"), (SQUARE_SIZE, SQUARE_SIZE))
        IMAGES[piece] = image
    
    # Trả về từ điển IMAGES đã được tạo ra
    return IMAGES

# Gọi hàm loadImages() để tải hình ảnh các quân cờ
IMAGES = loadImages()
